...
Cloudflare Pages（ダッシュボード・常時稼働・無料）
  ├── Next.js 静的サイト（過去記事・統計・設定を閲覧／編集）— 新聞風 Press モバイルデザイン（iPhone ファースト）
  │   ├── ホーム: マストヘッド（Tech Dispatch）・統計ストリップ・リード記事ヒーロー + 番号付き記事リスト
  │   ├── 過去記事: 今日/昨日/すべてフィルタタブ・日付グループ別番号付きリスト・ミニ反応ボタン
