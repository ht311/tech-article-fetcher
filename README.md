...
GitHub Actions (cron: 毎日 JST 8:00)
  ↓
Python スクリプト
  ├── RSS（日本語・企業・海外公式ブログ）/ Qiita から記事並列収集（優先トピックでプリフィルタリング）
  ├── 直近 24 時間にフィルタ・重複排除
  ├── Cloudflare KV からユーザー嗜好（過去フィードバック）を読み込み
  ├── Gemini API (gemini-2.0-flash → lite フォールバック) で TOP 5〜7 件を選定
  ├── LINE Messaging API（Flex Message 縦並びカード＋サムネイル）で送信
...
src/
├── main.py                      # エントリポイント
├── config.py                    # ソース一覧・定数（PREFERRED_TOPICS含む）
├── models.py                    # Pydantic データモデル（Article.thumbnail_url含む）
├── fetchers/
│   ├── rss_fetcher.py           # RSS/Atom フィード並列取得（サムネイル抽出対応）
│   ├── qiita_fetcher.py         # Qiita API 取得（タグ別並列取得対応）
│   └── devto_fetcher.py         # dev.to API 取得（サムネイル対応）
├── selector/
│   └── gemini_selector.py       # Gemini API による記事選定（優先トピック・モデルフォールバック対応）
├── notifier/
│   └── line_notifier.py         # LINE Push Message 送信（縦並び Flex Message）
└── storage/
