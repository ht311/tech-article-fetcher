...
_TOPICS_STR = "、".join(PREFERRED_TOPICS)

# Gemini に渡すシステムプロンプト。選定基準・除外基準・出力形式を定義する。
SYSTEM_PROMPT = f"""あなたはWebエンジニア向けの技術記事キュレーターです。
...
def _prefilter_articles(articles: list[Article], limit: int) -> list[Article]:
    """Gemini に渡す記事数を削減するプリフィルタリング。
    優先トピックにマッチする記事を優先し、残りを直近記事で補う。
    """
    ...
    def recency_key(a: Article) -> datetime:
        return a.published_at if a.published_at else epoch

    sorted_articles = sorted(articles, key=recency_key, reverse=True)
